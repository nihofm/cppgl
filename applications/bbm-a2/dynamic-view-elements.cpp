...
void init_dynamic_prototypes() {
    { // load player prototype
        ...
        // HINT: we may have skipped something regarding player colors
#ifndef A2_3
        Player::colors.resize(8);
        Player::colors[0] = glm::vec3(1, 0, 0);
        Player::colors[1] = glm::vec3(0, 1, 0);
        Player::colors[2] = glm::vec3(0, 0, 1);
        Player::colors[3] = glm::vec3(1, 1, 0);
        Player::colors[4] = glm::vec3(0, 1, 1);
        Player::colors[5] = glm::vec3(1, 0, 1);
        Player::colors[6] = glm::vec3(1, 0.5, 0.5);
...
void Player::update() {
    ...
    // HINT: Right here we may have skipped something tagged 'character wobble'
...
#ifndef A2_3
        if (elem->name.find("sphere") != string::npos)
...
bool Box::to_destroy() { return exploding && explo_timer.look() > render_settings::box_explosion_duration; }
