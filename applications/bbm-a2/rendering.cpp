...
void deferred_debug_pass(const std::shared_ptr<Framebuffer>& gbuffer) {
    static auto shader = make_shader("deferred-debug", "shader/copytex.vs", "shader/deferred-debug.fs");
...
void deferred_shading_pass(const std::shared_ptr<Framebuffer>& gbuffer) {
    static auto shader = make_shader("deferred-lighting", "shader/copytex.vs", "shader/deferred-lighting.fs");
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
    shader->bind();
    setup_light(shader);
    shader->uniform("gbuf_depth", gbuffer->depth_texture, 0);
