...
GLuint compile_shader(GLenum type, bool throw_error, std::map<GLenum, std::string>& source_files, std::map<GLenum, size_t>& timestamps) {
    ...
    if (shaderCompiled != GL_TRUE) {
        ...
        std::string error_msg = "ERROR: Failed to compile shader: " + source_files[type] + ".\n" + log + "\nSource:\n";
        // get relevant lines
        std::string out;
...
std::string Shader::base_path;

Shader::Shader(const std::string& name) : NamedMap(name), id(0) {}

Shader::Shader(const std::string& name, const std::string& compute_source) : Shader(name) {
    set_compute_source(compute_source);
    compile(true);
}

Shader::Shader(const std::string& name, const std::string& vertex_source, const std::string& fragment_source) : Shader(name) {
    set_vertex_source(vertex_source);
    set_fragment_source(fragment_source);
    compile(true);
}

Shader::Shader(const std::string& name, const std::string& vertex_source, const std::string& geometry_source, const std::string& fragment_source)
    : Shader(name) {
    set_vertex_source(vertex_source);
...
void Shader::set_source(GLenum type, const std::string& filename) {
    source_files[type] = concat(base_path, filename);
}

void Shader::set_vertex_source(const std::string& filename) {
    set_source(GL_VERTEX_SHADER, filename);
}

void Shader::set_tesselation_control_source(const std::string& filename) {
    set_source(GL_TESS_CONTROL_SHADER, filename);
}

void Shader::set_tesselation_evaluation_source(const std::string& filename) {
    set_source(GL_TESS_EVALUATION_SHADER, filename);
}

void Shader::set_geometry_source(const std::string& filename) {
    set_source(GL_GEOMETRY_SHADER, filename);
}

void Shader::set_fragment_source(const std::string& filename) {
    set_source(GL_FRAGMENT_SHADER, filename);
}

void Shader::set_compute_source(const std::string& filename) {
    set_source(GL_COMPUTE_SHADER, filename);
}
...
void Shader::reload_if_modified() {
    for (const auto& entry : source_files) {
        if (get_timestamp(entry.second) != timestamps[entry.first]) {
            compile();
            return;
        }
    }
